...
export default function Home() {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log(file, 'uploadin change ')
    if (!file) return;
    ...
    console.log(blobUrl, 'send the request with this')
  ...
  const identifyThings = async () => {
    ...
    console.log(formData, ' inform ME ')
    ...
    try {
      ...
      console.log(response, 'Post it here ')
  
      // If the API call was successful, set the response
      if (response.ok) {
        console.log("File uploaded successfully");
        console.log(response.ok, 'truthy here NOW');
  ...
  function toggleThis(label: string) {
    ...
    console.log(showThis, 'show the picture Here');
  }
  console.log(toggleThis, 'LOGGED it OUT');
